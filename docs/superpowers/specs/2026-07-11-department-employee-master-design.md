...
표 + 상세 드로어. **`ag-grid`는 쓰지 않는다.** `package.json`에 의존성이 있으나 코드베이스 어디에서도 사용하지 않으며, 기존 화면은 전부 순수 `<table>` + 코어 CSS 클래스(`.tbl`/`.th`/`.td`)를 쓴다. 새 도메인만 ag-grid를 도입하면 패턴이 갈라진다.

표본은 `src/pages/settings/UserAdmin.vue` — 표 + 드로어 + `SearchSelect` 조합이 그대로 필요한 형태다.
