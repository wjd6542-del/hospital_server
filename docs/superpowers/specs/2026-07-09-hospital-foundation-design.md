...
`prisma/seed-rbac.js`가 권한 카탈로그의 단일 출처이며 21개를 정의한다. 프론트엔드는 그중
일부만 참조한다. **10개를 버리고 11개를 남긴다.**

- 삭제 (10): `ledger.view`, `ledger.edit`, `settlement.view`, `settlement.edit`,
  `support.view`, `support.edit`, `gameCompany.view`, `gameCompany.edit`,
  `vendor.view`, `vendor.edit`
- 유지 (11): `board.view`, `board.write`, `faq.view`, `faq.edit`, `usermanager.view`,
  `usermanager.create`, `usermanager.update`, `permission.user.view`,
  `permission.user.update`, `permission.menu.view`, `permission.menu.update`

유지 권한의 `group` 필드도 손본다. `faq.*`의 그룹이 `"CS 관리"`이므로 `"FAQ"`로 바꾸고,
`gameCompany`·`vendor`가 빠진 `"환경설정"` 그룹은 그대로 둔다.
...
프론트엔드의 `pages/alerts/`는 백엔드 `notification` 모듈과 **아무 관련이 없다.**
`stores/alerts.ts`는 `supportApi.alerts()`를 호출해 **미해결 CS 응대 티켓**을 가져오고,
`AlertsView.vue`는 support desk 카드를 그려 `/support/:code`로 링크한다.

프론트엔드는 `/notification/*` 엔드포인트를 한 번도 호출하지 않는다. 헤더의 종 아이콘,
`App.vue`의 30초 폴링, 홈 대시보드의 응대 위젯이 모두 `supportApi.alerts()`에 의존한다.

따라서 백엔드 `notification` 모듈(모델·서비스·라우트)은 유지하되, **프론트엔드의 알림 UI는
전부 삭제한다.** 실제 알림 화면은 백엔드 API를 소비하는 별도 과제로, 다음 사이클에서 만든다.
기반 정리 이후 `notification`은 프론트 소비자가 없는 상태가 된다. 이는 의도된 것이다.
...
- `components/layout/AppHeader.vue` — 종 아이콘, 드롭다운, `useAlertsStore` 참조 제거
- `App.vue` — `alerts.startPolling(30000)` / `stopPolling()` 및 관련 import 제거
- `pages/home/HomePage.vue` — 440줄 중 `ledgerApi` · `settlementApi` · `supportApi` ·
  `supportDeskApi`를 쓰는 위젯 전부 제거. 살아남는 것은 공지·알림 게시판의 최근 글 목록
  위젯뿐이다. 사실상 재작성에 가깝다. 지표 위젯은 넣지 않는다 (보여줄 데이터가 없다).
...
`/settlement/vendor`, `/settlement/gameco`, `/support/:deskCode`, `/exchange`, `/alerts`.
...
`pages/settings/SettingsView.vue`의 탭 배열에서 `gameco` · `vendor` · `supportdesk` 탭을
제거한다. `tag` 탭의 `perm`이 `["support.view", "faq.view"]`이므로 `"faq.view"`로 축소하고,
`active`의 초기값 `"gameco"`를 `"board"`로 바꾼다.

철거 후 남는 화면: 로그인, 대시보드(홈), 게시판, FAQ, 마이페이지, 계정 관리(계정 권한 ·
화이트 아이피), 환경설정(게시판 · FAQ 분류 · 태그 · 다국어).
...
### 옛 토큰 참조와 하드코딩 hex 정리

치환 대상은 두 종류다.

**옛 CSS 변수 참조.** 생존 파일들이 폐기될 토큰을 약 839회 참조한다. `--line-hard` 165회,
`--seal` 102회, `--font-pixel` 92회, `--ink` 91회 순이다. 대부분 1:1 기계적 치환이므로
`sed` 매핑 스크립트로 일괄 처리한 뒤 눈으로 검수한다. 매핑은 위 토큰 표의 "폐기되는 옛 토큰"
열을 그대로 따른다.

**하드코딩 hex.** 28개 `.vue` 파일에 hex 색상이 박혀 있다 (베이스 컴포넌트 11 + 레이아웃 2 +
유지 페이지 15). `#7a5cff`(아케이드 바이올렛)가 17회, `#ede9ff`가 15회 등장한다.
hex를 가진 나머지 7개 파일은 삭제 대상이므로 자연 소멸한다.
...
`tailwind.css`의 `@media print` 블록은 `#voucher-print-clone`(정산 전표 출력)을 대상으로 하는데,
이 ID는 **CSS에만 있고 DOM 어디에도 없다.** 블록을 제거하고 `.no-print` 규칙만 남긴다.
병원 서류 출력은 해당 도메인 사이클에서 새로 설계한다.

`.badge-in` / `.badge-out`(회수/지급)은 삭제 대상 페이지 전용이므로 제거한다.
`.badge-indigo`는 `BoardView.vue`의 "공지" 배지가 쓰므로 `.badge-info`의 별칭으로 남긴다.
...
6. 프론트 — 페이지 8개 + `stores/alerts.ts` + 고아 컴포넌트 3개 삭제,
   라우터 · 사이드바 · `SettingsView` 정리
7. 프론트 — `AppHeader` 종 제거, `App.vue` 폴링 제거, `HomePage` 재작성
8. 테마 — `tailwind.css` 토큰 재설계 + 다크 짝 + `tailwind.config.js` 매핑, 죽은 CSS 제거
9. 테마 — 생존 파일의 옛 토큰 참조(약 839회)와 하드코딩 hex를 새 토큰으로 치환
